...
function playPause() {
  ...
  let loader = document.getElementById("loader_banner");
  if (video) {
...
video.addEventListener("progress", updateLoadingBanner);
...
function updateLoadingBanner() {
  ...
  if (buffered.length > 0) {
    ...
    if (loadedPercentage <= 99) {
      document.getElementById("loader_banner").style.display = "none";
    }
...
let indexed_retryCount = 0;

function loadJsonFilesWithRetries2() {
  if (indexed_retryCount < 3) {
    //console.log('try num ' + indexed_retryCount);
    fetch("index.json")
      .then((response) => {
        if (!response.ok) {
          throw new Error('Index.json - Network response was not OK');
        }
        return response.json();
      })
      .then((data) => {
      ...
      .catch((error) => {
        ...
        indexed_retryCount++; // Increment retry count
        loadJsonFilesWithRetries2(); // Retry loading the JSON files
      });
  } else {
    // Show an alert if all retries fail
    alert("Failed to load JSON files after 3 attemptss");
  }
}
...
function showIndexed() {
  ...
  let total_index_sections = index.length
  let section = document.getElementById("index")
  let section_innerHTML = ""
  let wallpaper_count = 0.0



  for (i = 0; i < total_index_sections; i++) {
    let present_section_name = index[i]["name"]
    let present_section_wallpapers = index[i]["walls"]
    let total_present_wallpapers = present_section_wallpapers.length
    console.log(total_present_wallpapers);

    // console.log(present_section_name);
    let present_section_innerhtml = `<div class="indexed_wallpaper">
    <div class="indexed_heading">
      <h1>` + present_section_name + `</h1>
      <div class="indexed_a"><i class="fa-solid fa-arrow-right"></i></div>
    </div>
    <div class="indexed_container">
      <div class="outer_pading">
        <div class="indexed_container_padding">`




    for (j = 0; j < total_present_wallpapers; j++) {
      // console.log(present_section_wallpapers[j]);
      let present_wallpaper_code = present_section_wallpapers[j];
      ...
      wallpaper_count = wallpaper_count + 0.1;



      switch (present_wallpaper_type_short) {
        case "1":
          preciew_innerhtml = `<div class="indexed_image"><a target="_blank" href="image.html?value=` + present_wallpaper_code + `" ><img
        src="` + present_wallpaper_thumbnaiL + `" alt="wallpaper" id="wallpaper` + wallpaper_count + `"> </a></div>`;

          break;


        case "2":
          preciew_innerhtml = `<div class="i_container" onclick="playvideowallpaper(` + wallpaper_count + `,'` + video_thumbnail + `')"><i class="fa-solid fa-play indexed_i"></i></div><div class="loader-container" id="loader` + wallpaper_count + `"><div class="loader"></div></div>
        <a target="_blank" href="image.html?value=` + present_wallpaper_code + `" ><video poster="` + present_wallpaper_thumbnaiL + `" loop muted src="" id="wallpaper` + wallpaper_count + `"  class="indexed_wallpaper_img"></video> </a>`;
        ...
        <a target="_blank" href="image.html?value=` + present_wallpaper_code + `" ><video poster="` + present_wallpaper_thumbnaiL + `" loop muted src="" id="wallpaper` + wallpaper_count + `"></video> </a>`;
        ...
        case "P3":
          preciew_innerhtml = `<div class="i_container" onclick="playvideowallpaper(` + wallpaper_count + `,'` + video_thumbnail + `')"><i class="fa-solid fa-play"></i></div><div class="loader-container" id="loader` + wallpaper_count + `"><div class="loader"></div></div>
        <a target="_blank" href="image.html?value=` + present_wallpaper_code + `" ><video poster="` + present_wallpaper_thumbnaiL + `" loop muted  id="wallpaper` + wallpaper_count + `"></video> </a>`;
        ...
        src="` + present_wallpaper_thumbnaiL + `" alt="wallpaper"> </a>`;
      ...
      let present_wallpaper_div = `<div class="indexed_wallpaper_land ">
      ` + preciew_innerhtml + `</div>`;
      // <div class="details">
      ...
      present_section_innerhtml += present_wallpaper_div;
    ...
    section_innerHTML += present_section_innerhtml + `</div></div ></div ></div > `
  }

  section.innerHTML = section_innerHTML
...
let display = document.querySelector(".banner")

function toggleFullScreen() {
  if (!document.fullscreenElement) {
    if (display.requestFullscreen) {
      display.requestFullscreen();
      lockOrientationAndForceRepaint(display);
  ...
  function lockOrientationAndForceRepaint(element) {
...
function filterByType(data) {
  ...
  videowallpaperarray = randomizearr(videowallpaperarray)
...
function randomizearr(arr) {
  ...
  for (i = 1; i <= l; i++) {
    let r_index = Math.floor(Math.random() * (indexarr.length));
    let poppedindex = (indexarr[r_index]) - 1
    indexarr[r_index] = indexarr[(indexarr.length) - 1]
    indexarr.pop()
    // //console.log((indexarr.length))
    randomarr.push(arr[poppedindex])
...
function get_quality2(codee) {
  ...
  switch (preffered_banner_quality) {
    ...
    case "1080":
      return_value = give_max_least_quality2(codee, "1080p");

      break;

    case "480":
      return_value = give_max_least_quality2(codee, "480p");

      break;

    case "720":
      return_value = give_max_least_quality2(codee, "720p");
...
function changebanner() {
  ...
  // let presentvideothumb = originaljson[randomvideowallpaper]["thumbnail"];
  let presentvideothumb = originaljson[randomvideowallpaper]["Quality_thumbnail"];
  ...
  videoElement.poster = presentvideothumb;
...
function prevbanner() {
  ...
  if (randomIndex != 0) {
    ...
    randomIndex = videowallpaperarray.length - 1
...
function get_quality(codee) {
  ...
  switch (thumb_quality_select) {
    ...
    case "1080":
      return_value = give_max_least_quality(codee, "1080p");

      break;

    case "480":
      return_value = give_max_least_quality(codee, "480p");

      break;

    case "720":
      return_value = give_max_least_quality(codee, "720p");
...
function play_indexed_wallpaper(playindex) {
